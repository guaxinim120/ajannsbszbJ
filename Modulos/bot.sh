...
ajuda ()
{
    if [[ "${message_from_id[$id]}" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>BEM VINDO(a) AO MASTER DS MANAGER</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
        env_msg+="<i>LISTA DE COMANDOS</i>\n\n"
        ...
        env_msg+="[<b>17</b>] /relatorio = Informações\n"
        env_msg+="[<b>18</b>] /ajuda = Informações do bot\n"
        ...
        local env_msg1
        env_msg1="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg1+="<b>BEM VINDO(a) MASTER DS MANAGER</b>\n"
        env_msg1+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
        env_msg1+="<i>LISTA DE COMANDOS</i>\n\n"
        ...
        env_msg1+="[<b>10</b>] /relatorio = informações\n"
        env_msg1+="[<b>11</b>] /ajuda = Informações do bot\n"
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        ...
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e "🚫 VOCÊ ESTÁ SUSPENSO 🚫\n\nCONTATE O ADMINISTRADOR")"
        return 0
    else
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
...
fun_revenda() {
    ...
    local env_msg1
    env_msg1="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
    env_msg1+="<b>MENU REVENDA BOT MASTER DS MANAGER</b>\n"
    env_msg1+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    env_msg1+="<i>LISTA DE COMANDOS</i>\n\n"
...
{
    local msg
    msg="Olá <b>${message_from_first_name[$id]}</b>\nSeja bem-vindo(a)\n\n"
    msg+="USE O COMANDO [ /menu ] PARA MOSTRAR A LISTA DE COMANDOS\n\n"
    ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e $msg)" \
        --parse_mode html
    return 0    
...
verifica_suspenso() {
    ...
    [[ -d /etc/bot/suspensos/$_RevCliente ]] && {
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e "⚠️ VOCÊ ESTÁ SUSPENSO ⚠️\n\nCONTATE O ADMINISTRADOR")"
        return 0
    }
}
...
fun_del_rev() {
    ...
    ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
        --text "🗑 REMOVER REVENDEDOR 🗑\n\nInforme o user dele [Ex: @masterdesignerp]:" \
        --reply_markup "$(ShellBot.ForceReply)"
}
        ...
        break
            ...
            case ${message_reply_to_message_text[$id]} in
                    ...
                    '👥 ADICIONAR REVENDEDOR 👥\n\nInforme o nome:')
                        echo "Nome: ${message_text[$id]}" > $CAD_ARQ
                        ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
                            --text 'Informe o user dele [Ex: @masterdesignerp]:' \
                            --reply_markup "$(ShellBot.ForceReply)"
                        ;;
                    'Informe o user dele [Ex: @masterdesignerp]:')
                        ...
                        [[ -z $_VAR1 ]] && {
                            ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                                --text "$(echo -e "❌ Erro \n\n⚠️ Informe o user [EX: @masterdesignerp]")" \
                                --parse_mode html
                                break
                        }
